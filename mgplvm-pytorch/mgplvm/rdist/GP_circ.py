...
class GP_circ(GPbase):
    ...
    @property
    def prms(self):
        return self.nu, self.scale, self.ell, self.c, self.nu2, self.alpha
    ...
    def kl(self, batch_idxs=None, sample_idxs=None):
        ...
        nu, S, c, nu2 = self.nu, self.scale, self.c, self.nu2
        alpha = self.alpha
        ell = self.ell
        if sample_idxs is not None:
            nu = nu[sample_idxs, ...]
        ...
        MeanTerm = torch.square(nu).sum(-1) + torch.square(nu2).sum(-1) #(n_samples x d)
        DimTerm = S.shape[-1]
