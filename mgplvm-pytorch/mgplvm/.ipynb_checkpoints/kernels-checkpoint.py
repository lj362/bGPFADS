...
class Linear(Kernel):
    ...
    def __init__(self,
        ...
        distance : function
            function used to compute distances
        d : int
            number of latent dimensions/inputs
        alpha : Optional[array]
            initialization of the weight prior std
        learn_weights : Optional[bool]
            if True, a full nxd weight matrix is learned
        learn_alpha : Optional[bool]
            if True, a scale is learned for each neuron (std of the prior over weights; shared across latents for each neuron)
        ard : Optional[bool]
            if True, a length scale is learned for each latent dimension (shared across neurons for each latent)
        Y : Optional[array]
            data used for smart initialization of dims (n_samples x n x my)
        """
